...
- `probably::int_conformal_full()` is not supported for multistep step views: its refit-per-candidate design would need to substitute a candidate value into the single *raw* column `step_lead()` derives every step's truth from, which then shifts every nearby row's target — a materially different problem than the single-column substitution `kerasnip_output_view()` uses, and out of scope for now.
