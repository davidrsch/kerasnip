...
Plain `saveRDS()` does **not** preserve the underlying Keras model weights.
Use `bundle::bundle()` before saving to make a fitted workflow fully portable:
...
saveRDS(bundled, "my_model.rds")

# Reload in a new R session
library(kerasnip)
library(bundle)
fit_wf <- unbundle(readRDS("my_model.rds"))
predict(fit_wf, new_data = new_data)
```

`predict()` will automatically restore the parsnip model registration — no
extra step is needed after `unbundle()`.
