...
# Limit CPU usage for CRAN checks
...
`kerasnip` serializes the Keras model weights to bytes at fit time and stores
them alongside the workflow object. This means plain `saveRDS()` / `readRDS()`
**works out of the box** — the underlying Keras model is restored automatically
the first time `predict()` is called on the reloaded object.
...
`vetiver` or other MLOps tools, use `bundle::bundle()` instead:
...
See `vignette("saving_and_reloading")` for a detailed comparison of both
approaches.
