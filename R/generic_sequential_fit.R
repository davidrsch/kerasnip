#' Generic Keras Model Fitting Implementation
#'
#' @description
#' This function is the internal engine for fitting models generated by `create_keras_sequential_spec()`.
#' It is not intended to be called directly by the user.
#'
#' @details
#' This function performs several key steps:
#' \enumerate{
#'   \item \strong{Argument & Data Preparation:} It resolves arguments from `parsnip`
#'     and prepares the `x` and `y` data for Keras. It automatically determines
#'     the `input_shape` from `x` and, for classification, the `num_classes` from `y`.
#'   \item \strong{Dynamic Model Construction:} The user is responsible for defining the
#'     entire model architecture via `layer_blocks`. The function iterates through
#'     the blocks in the order they are provided:
#'     \itemize{
#'       \item The first block function \strong{must initialize the model}, typically
#'         by calling `keras3::keras_model_sequential()`. It can accept an
#'         `input_shape` argument, which will be provided automatically.
#'       \item Subsequent blocks receive the model and add layers to it.
#'       \item An output layer block can accept a `num_classes` argument, which is
...
#'     \itemize{
#'       \item Override defaults by passing arguments prefixed with `compile_`
#'         (e.g., `compile_loss = "mae"`, `compile_optimizer = "sgd"`).
#'     }
