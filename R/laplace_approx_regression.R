...
#' Per-Sample Epistemic (Confidence) Variance
#'
#' @description
#' Computes the per-sample epistemic variance under the diagonal Laplace
#' posterior.
#'
#' @details
#' Formula:
#' ```
#' sigma^2_epistemic(x*) = sum_j phi_j(x*)^2 / (N * h_j / sigma_sq + tau)
#'                       + 1 / (N / sigma_sq + tau)
#' ```
#' The last term is the bias variance (h_bias = 1).
#'
#' @param features An (M x d) numeric matrix of penultimate-layer features for
#'   new test points.
#' @param h_diag Numeric vector of length (d + 1).  Diagonal GGN entries
#'   (weights then bias).
#' @param tau Scalar prior precision.
#' @param sigma_sq_noise Scalar observation noise variance.
#' @param n_training Integer, number of training points used to compute h_diag.
#' @return A numeric vector of length M: per-sample epistemic variances.
#' @noRd
epistemic_var_regression <- function(
  features,
  h_diag,
  tau,
  sigma_sq_noise,
  ...
  h_weights <- h_diag[1L:d_features]
  h_bias <- h_diag[d_features + 1L]

  prec_weights <- n_training * h_weights / sigma_sq_noise + tau
  prec_bias <- n_training * h_bias / sigma_sq_noise + tau

  var_weights <- 1 / prec_weights
  var_bias <- 1 / prec_bias

  epi_var <- features^2 %*% var_weights + var_bias
  as.vector(epi_var)
}


#' Per-Sample Predictive Variance
#'
#' @description
#' Computes the per-sample predictive variance: epistemic variance plus
#' observation noise.
#'
#' @inheritParams epistemic_var_regression
#' @return A numeric vector of length M: per-sample predictive variances.
#' @noRd
predictive_var_regression <- function(
  features,
  h_diag,
  tau,
  sigma_sq_noise,
